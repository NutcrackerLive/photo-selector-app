...
export default function ProjectView() {
  ...
  const getExportPath = useMutation({
    mutationFn: () => invoke<string>(IPC_CHANNELS.PHOTOS_GET_EXPORT_PATH, projectId),
    onSuccess: (path) => {
      setExportPath(path);
  ...
  const changeExportLocation = async () => {
    const newPath = await invoke<string>(IPC_CHANNELS.DIALOG_SELECT_FOLDER);
    if (newPath) {
  ...
  const exportSelected = useMutation({
    mutationFn: () => invoke<{ count: number; skipped: number; exportPath: string }>(
      IPC_CHANNELS.PHOTOS_EXPORT,
      { projectId, exportPath }
    ),
    onSuccess: (data) => {
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => getExportPath.mutate()}
                  disabled={selectedCount === 0 || exportSelected.isPending || getExportPath.isPending}
      ...
      {showExportModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Export Photos</h3>

            <p className="text-sm text-gray-600 mb-4">
              {selectedCount} photos will be exported as optimized JPEGs.
            </p>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg font-medium transition-colors disabled:opacity-50"
              >
